...
class CameraTracker:
    def __init__(self, camera_id):
        ...
        # Enhanced parameters - ADJUST THESE FOR YOUR BLOBS
        self.CONTOUR_UPDATE_INTERVAL = 0.01  # Reduced for smoother tracking but more CPU intensive
        self.DISTANCE_THRESHOLD = 200.0      # More sensitive tracking at lower value
        self.MIN_CONTOUR_AREA = 2000        # Minimum blob area
        self.MAX_CONTOUR_AREA = 10000      # Maximum blob area

        # Morphological kernel for noise reduction
        self.morph_kernel = cv.getStructuringElement(cv.MORPH_ELLIPSE, (5, 5))

    def enhanced_preprocessing(self, frame):
        """Enhanced preprocessing for stable edge detection"""
        ...
        # Apply adaptive thresholding instead of fixed Canny
        binary = cv.adaptiveThreshold(filtered, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, 
                                    cv.THRESH_BINARY_INV, 11, 2)

        # Morphological operations for noise reduction
        # Opening to remove noise
        opened = cv.morphologyEx(binary, cv.MORPH_OPEN, self.morph_kernel, iterations=1)

        # Closing to fill gaps
        closed = cv.morphologyEx(opened, cv.MORPH_CLOSE, self.morph_kernel, iterations=2)

        return closed

    def select_stable_contour(self, contours):
        """Select contour using multiple criteria for stability"""
        if not contours:
            return None
        ...
        # If we don't have a template, use largest valid contour
        if not self.tracking_locked:
            best_contour = max(valid_contours, key=cv.contourArea)
            self.tracked_contour = best_contour
            self.tracking_locked = True
            return best_contour

        # Find contour most similar to previous one
        best_contour = None
        best_similarity = float('inf')

        for contour in valid_contours:
            similarity = self.calculate_contour_similarity(self.tracked_contour, contour)
            if similarity < best_similarity:
                best_similarity = similarity
                best_contour = contour

        # Update template if similarity is reasonable
        if best_similarity < 0.5:  # Adjust threshold as needed
            self.tracked_contour = best_contour

        return best_contour

    def calculate_contour_similarity(self, contour1, contour2):
        """Calculate similarity between two contours"""
        if contour1 is None or contour2 is None:
            return float('inf')

        try:
            # Shape similarity using matchShapes
            shape_diff = cv.matchShapes(contour1, contour2, cv.CONTOURS_MATCH_I1, 0.0)
            ...
            # Position similarity
            center1 = self.get_center(contour1)
            center2 = self.get_center(contour2)
            pos_diff = np.sqrt((center1[0] - center2[0])**2 + (center1[1] - center2[1])**2) / 100.0

            # Combined similarity (weighted)
            similarity = 0.4 * shape_diff + 0.3 * area_diff + 0.3 * pos_diff
    ...
    def process_contours(self, frame):
        """Enhanced contour processing with stability improvements"""
        # Use enhanced preprocessing
        processed = self.enhanced_preprocessing(frame)

        # Find contours
        contours, _ = cv.findContours(processed, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)

        # Select stable contour
        current_contour = self.select_stable_contour(contours)

        current_time = time.time()
        update = False

        if current_contour is not None and (current_time - self.last_contour_update_time) >= self.CONTOUR_UPDATE_INTERVAL:
            # Get center and apply temporal smoothing
            raw_center = self.get_center(current_contour)
            smoothed_center = self.temporal_smoothing(raw_center)

            if self.last_drawn_center is None:
                update = True
            else:
                # Check if blob has moved significantly
                dx = smoothed_center[0] - self.last_drawn_center[0]
                dy = smoothed_center[1] - self.last_drawn_center[1]
                dist_moved = np.sqrt(dx ** 2 + dy ** 2)
            ...
            if update:
                self.last_drawn_contour = current_contour
                self.last_drawn_center = smoothed_center

                # Update distance traveled
                if self.prev_center is not None:
                    dy_total = self.last_drawn_center[1] - self.prev_center[1]
                    self.total_distance += abs(dy_total)  # Use absolute value for total movement
        ...
        if self.last_drawn_contour is not None and self.last_drawn_center is not None:
            # Draw contour with varying color based on tracking confidence
            color = (0, 255, 0) if current_contour is not None else (0, 128, 0)
            cv.drawContours(frame, [self.last_drawn_contour], -1, color, 2)
            cv.circle(frame, self.last_drawn_center, 5, (0, 0, 255), -1)
            ...
            # Show blob area if available
            if self.last_drawn_contour is not None:
                area = cv.contourArea(self.last_drawn_contour)
                cv.putText(frame, f"Area: {area:.0f}px", (10, 60), 
...
def binarize_grayscale(image, threshold=50):
    """Convert grayscale image to pure black and white (0 or 255)."""
    ...
    def update_video(self):
        ...
        if ret1:
            if self.monitoring_active:
                ...
                gray1 = binarize_grayscale(gray1, threshold=28) # ← add quantization here
                frame1 = cv.cvtColor(gray1, cv.COLOR_GRAY2BGR)

                frame1 = self.camera1_tracker.process_contours(frame1) # Step 3: Draw contours/edges in color
            else:
                gray1 = cv.cvtColor(frame1, cv.COLOR_BGR2GRAY)
                frame1 = cv.cvtColor(gray1, cv.COLOR_GRAY2BGR)
        ...
        if ret2:
            if self.monitoring_active:
                ...
                gray2 = binarize_grayscale(gray2, threshold=28)
                frame2 = cv.cvtColor(gray2, cv.COLOR_GRAY2BGR)
                frame2 = self.camera2_tracker.process_contours(frame2)
            else:
                gray2 = cv.cvtColor(frame2, cv.COLOR_BGR2GRAY)
                frame2 = cv.cvtColor(gray2, cv.COLOR_GRAY2BGR)
